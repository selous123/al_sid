...
class DataProcess:
    ...
    def __call__(self, example):
        #user_history = [int(i) for i in example[self.input_column].split(',')] 
        ...
        if len(user_history) < self.max_length:
            ...
            pad_item = [0] * len(user_history[0]) if user_history else [0]  
...
def custom_collate(batch):
    # Step 1: user_history  List[List[List[int]]] → (B, L, C) tensor
...
if __name__ == '__main__':
    dataset = load_dataset("csv", data_files="xxx/data/AL-GR-Tiny/u2i/s1_tiny.csv", split='train', streaming=True)
